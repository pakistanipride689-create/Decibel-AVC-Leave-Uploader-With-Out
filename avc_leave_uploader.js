...
(async()=>{
    ...
    c.style="position:fixed;top:5%;left:50%;transform:translateX(-50%);background:#fff;padding:40px 30px 20px 30px;z-index:999999;box-shadow:0 0 40px 10px rgba(0,0,0,0.8);border-radius:20px;text-align:center;font-family:sans-serif;min-width:500px;";
    c.innerHTML=`
        ...
        <div style='margin-top:25px;text-align:left;'>
            <button id='closeBtn' style='background:maroon;color:#fff;padding:7px 18px;border:none;border-radius:8px;cursor:pointer;'>Close</button>
        </div>
    `;
    ...
    document.getElementById('dl').onclick=()=>{
        try{
            ...
            let csv="Emp ID,Attendance Date,Leave Type,Remarks\n";
    ...
    document.getElementById("up").onchange=async e=>{
        ...
        function next(){
            if(i>=rows.length){
                console.log("✅ Done");
                if(rejected.length)downloadRejected();
                c.remove();
                return;
            }
            ...
            setTimeout(()=>{
                ...
                trList.forEach(row=>{
                    ...
                    if(txt.includes(id)&&txt.includes(date)){
                        ...
                        let remarkBox=inputs[inputs.length-1];
                        if(remarkBox){remarkBox.value=remark;remarkBox.dispatchEvent(new Event("input",{bubbles:true}));}
                        matched=true;
                    }
                });
